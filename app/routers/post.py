...
def get_posts(db: Session = Depends(get_db)):
    ...
    # par défaut, le join de SQLAlchemy est un left inner join
...
# l'ordre est important, si on met le posts/{id} avant,
# le mot latest sera interprété comme du string dans la methode get_post
# et retournera une erreur qui demande que le id soit un int
...
def delete_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if post == None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"post id {id} does not exist")
    ...
    # see 'Selecting a Synchronize Strategy' for more info
